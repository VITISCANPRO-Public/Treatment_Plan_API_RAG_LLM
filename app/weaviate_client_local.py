...
print(">>> WEAVIATE CLIENT LOCAL CHARGÉ <<<")
...
def weaviate_client():
    ...
    rest_host = os.getenv("WEAVIATE_REST_HOST")
    grpc_host = os.getenv("WEAVIATE_GRPC_HOST")
    api_key = os.getenv("WEAVIATE_API_KEY")

    if not rest_host:
        raise ValueError("WEAVIATE_REST_HOST manquant dans .env")
    if not grpc_host:
        raise ValueError("WEAVIATE_GRPC_HOST manquant dans .env")

    rest_host = rest_host.replace("https://", "").replace("http://", "").rstrip("/")
    grpc_host = grpc_host.replace("grpc://", "").replace("https://", "").replace("http://", "").rstrip("/")
